...
results = []
count = 0
for batch in generate_batches(task1_chunk, questions, batch_size):
  ordered_batch = scheduler(batch)
...
df.to_csv(f"task2_result/task2_1 results.csv", index=False)
...
plt.tight_layout()
plt.show()
plt.savefig(f"task2_result/task2_1_plot.png")
